...
import logging
logging.basicConfig(filename='bioconv2d_dev.log',
...
def same_padding(
    ...
    """Computes the amount of padding needed to keep the output of a Conv2D layer of the same size as the input, given the 
    `kernel_size` of the filters and the amount of `dilation`. Stride is assumed to be 1, as no other values are supported by PyTorch 
    (see F.conv2d docs).
    Note: if kernel_size is even along an axis, and dilation is odd along that same axis, the required padding will be asymmetric. 
    In that case, additional padding is added to the right/bottom.
    Returns
    -------
...
def update_filters(img: torch.Tensor,
    ...
    if groups > 1:
        ...
        # Example of usage: > torch.arange(11).chunk(6)
        # > (tensor([0, 1]),
        # >  tensor([2, 3]),
        # >  tensor([4, 5]),
        # >  tensor([6, 7]),
        # >  tensor([8, 9]),
        # >  tensor([10]))
        # Splits the image along the channel dimension
        img_groups = torch.chunk(img, groups, dim=1)
        # Splits the filters along the 'in_channels' dimension
    ...
    # the -1 puts all the parameters of each kernel in a single row
    weights = filters.reshape(out_channels, -1)
    #weights.shape = (out_channels, n_parameters_per_kernel)
    # There are `out_channels` filters (kernels) that are applied to the input, each with weights.shape[-1] parameters,
    # one for each pixel in their local receptive field.
    ...
    # preliminary_padding applies to a tensor `x` any padding that cannot be applied directly by F.conv2d
    # (e.g. asymmetric padding, or when padding_mode is not 'zeros').
    img, added_padding, new_padding = preliminary_padding(
        img, padding, kernel_size, dilation, padding_mode)

    # This is what differs from the proper biopytorch implementation
    # TORCH.NN.FUNCTIONAL.UNFOLD:
    # Extracts sliding local blocks from a batched input tensor.
    # More mathematical details on the operation permoremed can be found in https://pytorch.org/docs/stable/generated/torch.nn.Unfold.html#torch.nn.Unfold
    # Currently, only 4-D input tensors (batched image-like tensors) are supported.
    patches = F.unfold(img, kernel_size=(kernel_height, kernel_width), stride=stride,
                       padding=new_padding, dilation=dilation)  # Extract all the patches for the input images
    #Shape is (minibatch, n_parameters_per_kernel, n_patches)
    # img.shape was (minibatch, channels, height, width)
    # n_parameters_per_kernel should be channels * kernel_height * kernel_width
    logging.info(
        f"patches.shape = {patches.shape} (minibatch, n_parameters_per_kernel, n_patches)")
    ...
    # n_all_patches_in_minibatch = minibatch * n_patches
    logging.info(
        f"patches.shape = {patches.shape} (n_parameters_per_kernel, n_all_patches_in_minibatch)")

    # "Manual" conv2d using a Lebesgue p-norm.
    currents = torch.matmul(torch.sign(weights) *
                            torch.abs(weights) ** (lebesgue_p - 1), patches)
    # Basically, each kernel (total: `out_channel`) is applied to a patch (total: `n_patches`), producing a scalar activation.
    #Shape is (out_channels, n_all_patches_in_minibatch)
    logging.info(
        f"currents.shape = {currents.shape} (out_channels, n_all_patches_in_minibatch)")

    #currents = currents.view(-1, currents.shape[-1])

    # ranking_indices[k][i] contains the index of the kernel with the k-highest
    _, ranking_indices = currents.topk(ranking_param, dim=0)
    # activation when applied to the i-th patch.
    #Shape is (ranking_param, n_all_patches_in_minibatch)
    logging.info(
        f"ranking_indices.shape = {ranking_indices.shape} (ranking_param, n_all_patches_in_minibatch)")

    post_activation_currents = torch.zeros_like(currents)
    #Shape is (out_channels, n_all_patches_in_minibatch)
    batch_indices = torch.arange(patches.size(-1))
    # The kernels that are most active for a patch are updated towards it
    post_activation_currents[ranking_indices[0], batch_indices] = 1.0
    # Others that are less activated are pushed
    post_activation_currents[ranking_indices[ranking_param - 1],
                             batch_indices] = -delta
    # in the opposite direction (i.e. they will be "less active" when encountering a similar patch in the future)
    logging.info(
        f"post_activation_currents.shape = {post_activation_currents.shape} (out_channels, n_all_patches_in_minibatch)")

    delta_weights = torch.matmul(post_activation_currents, torch.t(patches))
    #Shape is (out_channels, n_parameters_per_kernel)
    logging.info(
        f"delta_weights.shape = {delta_weights.shape} (out_channels, n_parameters_per_kernel)")

    # torch.mul applies element-wise multiplication, summing we obtain the hebbian terms
    ...
    # .sub_ is the inplace version of tirch.sub: torch.sub(input, other, *, alpha=1, out=None) → Tensor
    # Subtracts other, scaled by alpha, from input (out_i = imput_i - alpha x other_i)
...
def preliminary_padding(x: torch.Tensor,
    ...
    if symmetric:
        ...
        if padding_mode != 'zeros':  # F.pad must be used
            ...
            x = F.pad(x, (pad_width, pad_width, pad_height,
                      pad_height), mode=padding_mode)
        ...
        if padding_mode == 'zeros':
            # because F.pad uses a different convention...
...
def update_filters_conv(img: torch.Tensor,
    ...
    logging.info(
        f"Received img.shape = {img.shape} and filters.shape = {filters.shape}")
    ...
    currents = F.conv2d(img, filters_p, stride=stride,
                        padding=new_padding, dilation=dilation)
    ...
    logging.info(
        f"post_activation_currents.shape = {post_activation_currents.shape}")

    # We want to weight each patch in the minibatch by the corresponding post_activation_current,
    # and compute their weighted sum. This is done by using a 2d convolution with a specially designed filter,
    # such that each entry in `activation_filter` is applied exactly to the entries of a single patch
    ...
    # 1 0 1
    # 2 1 0
    # Focus on just one entry of this filter, e.g. the first "1" (at (1, 1) position). When the filter is applied during the 2d convolution,
    # this "1" overlaps exactly with the first patch, i.e.
    # 0 1
    # 1 0
    ...
    # 2 0 3
    # 0 0 0
    # In the general case, this can be constructed by constructing a block matrix, where each block has a unique non-zero value in
    # the top-left given by a weight, and the block size is the stride. So, for a (2, 2) stride, we have 2x2 blocks:
    # 2 -> 2 0
    #      0 0
    # For stride = (3, 3):
    # 5 -> 5 0 0
    ...
    # 2 0 3 0
    # 0 0 0 0
    # Which must be "cropped" to the correct size. We know that, after the 2d conv, the resulting shape should be that of the filters.
    # So we can invert the formula for the output shape of F.conv_2d to find the correct dimensions for the kernel, which in this case are 2x3.

    # In the case of batches, we use the batch dimension as the channels one. It works.

    # Shape is (in_channels, batch_size, height, width)
    separated_images = img.transpose(0, 1)
    ...
    stride_mask = torch.zeros(
        stride[0], stride[1], device=img.device, dtype=img.dtype)
    stride_mask[0][0] = 1
    activation_filter = torch.kron(
        activation_filter, stride_mask)  # construct a block matrix
    ...
    delta_weights = F.conv2d(separated_images, activation_filter,
                             padding=new_padding, stride=dilation).swapaxes(0, 1)
