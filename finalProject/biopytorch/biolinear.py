...
class BioLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return F.linear(
            ...
            torch.abs(self.weight) ** (self.lebesgue_p - 1),
            self.bias,
        )
    ...
    def delta_weights(self, x: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
        ...
        currents = torch.matmul(
            torch.sign(weights) *
            torch.abs(weights) ** (self.lebesgue_p - 1), x
    ...
    def training_step(self, x: torch.Tensor, learning_rate: float = 0.1) -> float:
        ...
        if self.bias is not None:
            ...
            # problem here
            self.weight.add_(learning_rate * delta_weights[:, 1:])
        ...
        convergence = torch.max(
            torch.abs(norm - torch.ones_like(norm))).cpu().numpy()
