...
class BioConv2d(nn.Module):
    def __init__(
        ...
        # TODO The only not implemented thing is the bias. This can be solved by adding a batchnorm (even without learnable parameters, i.e. with affine=False) before this layer. 
        # In fact, I'm not sure how the Krotov learning rule can be generalized to biases, since the original paper does not use them.
        #  Perhaps simply adding ones in the input? Then how to do this in Conv2d layers? I'll leave this problem for a future version...
    ...
    def delta_weights(self, x: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        ...
        delta_weights = F.conv2d(
            ...
            0, 1
        )  # setting stride=dilation gives for free support for the dilation argument, in the sense that it gives equal results with the unfold-version code. 
        #However, this should be checked a bit better, as I don't have a full theoretical understanding on why this works in practice.
